...
| `work/hns-wallet-rs` | `604a35771a9427696b6ecf533368205392e62979` | prior wallet/name/value containment remains; private ABI v2 now uses one authority-handle/revision, wallet-session, permission-generation binding for results, prompts, and events, preserves nonzero permission tombstones, exposes a private typed capability snapshot over the canonical 43-name vocabulary, and leaves `hns_requestAccounts` unavailable; configured `origin` remains seven commits behind this unpushed local head, source/static review only was performed, the added tests remain unrun, no push is authorized, and no qualification result exists |
