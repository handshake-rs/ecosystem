...
| `work/hns-wallet-rs` | `b999b330f17a0d7b300bff5e96b5ec2fbd294ec6` | prior private ABI, Bitcoin swap derivation, provider hardening, exact quote adoption, and fail-closed legacy Shakedex gates remain; an encrypted bounded `HnsName` scan shares the ordinary scan's exact chain/mempool snapshot and authoritative-account revision/CAS ordering rejects derivation rollback, but names remain watch-only; Ethereum advertises offline receive only, with synchronization/history/send/value/settlement/mainnet unavailable behind false gates and opaque unissued permits, and raw signed bytes remain zeroizing/opaque/redacted; configured `origin` remains six commits behind this unpushed local head, no push is authorized, and no new qualification result exists |
...
| `work/hns-dane-browser-extension` | `d58e1473e47bf9a50faafa5a05bba74756bdf314` | private wallet ABI/service protocol 2 discovery with website provider schema 1, the closed 12-kind browser-owned approval projection, canonical approval IDs, private-authority containment, native-only event routing, and exact close/reject revalidation are committed; every artifact-authenticity, transport, runtime, engine-authority, provider, and value gate remains false without the signed service and reviewed adapters |
| `work/hns-dane-browser-mobile` | `d6df646df0989b7ab25bc305c0ae80d2f08ce432` | dormant Android/iOS website-schema-1/private-native-ABI-2/public-approval-schema-2 projections close twelve approval summaries and thirteen typed events; source/static inspection only, with no build, test, simulator, device, or install result; local `main` is unpushed, all four release gates remain false, the unavailable adapter remains hardwired, and controller/wallet-runtime/wallet-FFI/generated-binding/UI/event-producer wiring is absent |
