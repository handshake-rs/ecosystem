...
| 23 | Chromium extension builds | PARTIAL: exact-head hosted CI passes repository/supply-chain, 75-test extension, Rust/native-host, packaging, and workflow gates; v0.5.4 publishes cross-platform MV3/native-host/Setup artifacts, protected credentialed jobs signed/notarized both macOS architectures, and a default-branch publisher reverified the replaced assets; the publisher's `release` environment still lacks protection rules, installed catalog/browser proxy/restart/uninstall/policy-revocation matrices remain unrun, and Windows artifacts remain unsigned |
